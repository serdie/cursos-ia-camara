...
import { CatoAvatar } from './cato-avatar';
...
interface CatoChatProps {
  initialQuestion?: string;
}

/**
 * CatoChat Component
 * Componente principal del chat con el asistente Cato
 * Conectado a la API de IA para responder preguntas reales
 */
export function CatoChat({ initialQuestion }: CatoChatProps) {
  ...
  useEffect(() => {
    if (isOpen && messages.length === 0) {
      setMessages([{
        id: 'welcome',
        role: 'assistant',
        content: '¡Hola! 👋 Soy Cato, tu asistente virtual del curso de IA de la Cámara de Comercio de Toledo.\n\nPregúntame sobre:\n• Los módulos del curso\n• Conceptos de inteligencia artificial\n• Cómo usar la plataforma\n• Dudas técnicas\n\n¿En qué puedo ayudarte hoy?',
      }]);
    }
  }, [isOpen]);
  ...
  const handleSendMessage = async (e: React.FormEvent | null, customMessage?: string) => {
    ...
    try {
      // Llamar a la API de Cato
      const response = await fetch('/api/cato-chat', {
        method: 'POST',
        headers: {
      ...
      if (contentType && contentType.includes('application/json')) {
        ...
        console.error('Error en API Cato (no JSON):', textError);
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50">
        ...
        <div className="absolute bottom-16 right-0 w-96 bg-background border rounded-xl shadow-2xl overflow-hidden animate-slide-up">
          ...
          <div className="p-4 border-b bg-gradient-to-r from-primary to-accent">
            <div className="flex items-center gap-3">
              <CatoAvatar state={avatarState} size="sm" />
              <div>
                <h3 className="font-semibold text-primary-foreground">Asistente Cato</h3>
      ...
      >
        <CatoAvatar state={isOpen ? avatarState : 'idle'} size="sm" />
        <span className="font-medium">{isOpen ? 'Cerrar' : 'Cato'}</span>
      </button>
    </div>
  );
}
